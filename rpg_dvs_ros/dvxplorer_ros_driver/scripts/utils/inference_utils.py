...
class UnsharpMaskFilter:
    ...
    def __init__(self, options, device):
        self.unsharp_mask_amount = options.unsharp_mask_amount # 0.3
        self.unsharp_mask_sigma = options.unsharp_mask_sigma # 1
        self.gaussian_kernel_size = 5
        self.gaussian_kernel = gkern(self.gaussian_kernel_size,
                                     self.unsharp_mask_sigma).unsqueeze(0).unsqueeze(0).to(device) #[1,1,5,5] shape
...
def events_to_voxel_grid_pytorch(events_torch, num_bins, width, height, device):
    """
    Build a voxel grid with bilinear interpolation in the time domain from a set of events.
